...
Use the [Word Frequency Counter](/tools/word-frequency-counter/) to analyze blog posts, landing pages, product descriptions, documentation, notes, transcripts, or copied text.
...
```text
Format JSON. Validate JSON. Minify JSON.
```

The word `json` appears three times. The words `format`, `validate`, and `minify` appear once each.
...
2. Review the top repeated words.
3. Ignore necessary topic terms.
4. Look for filler, vague words, or repeated adjectives.
5. Use the [Word Counter](/tools/word-counter/) to check overall length.
6. Use the [Text Diff Checker](/tools/text-diff/) after editing to review what changed.
